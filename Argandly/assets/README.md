...
3. **Output Extraction**: Read Cartesian coordinates or Polar metrics (\(\text{magnitude } r, \text{phase } \theta\)) directly from the synchronized tabular view below the canvas.
...
| **Line-by-line syntax:**        |                                      |
| `1.5 - 4e-2i x2`                | `[1.5 - 0.04i, 1.5 - 0.04i, -2, -2]` |
| `-2ones(1, 2)`                  |                                      |
...
\[
\omega_k^{(m+1)}=\omega_k^{(m)}-\frac{1}{\frac{P'(\omega_k^{(m)})}{P(\omega_k^{(m)})}-
...
* **Isolated Memoization Cache**: Configures an internal Least Recently Used (LRU) cache layer tracking past complex inputs. This retains precomputed root arrays for configurations \(1 \dots N\) to provide instantaneous rendering switches.
    * *Bounds*: \(1 \dots 8\) unique root structures (Default: \(3\)).
...
*This software is proprietary. Unauthorized copying, distribution, or modifications are strictly prohibited.*
